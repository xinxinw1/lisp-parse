/***** Lisp Parser Devel *****/

/* require tools >= 3.1 */
/* require lisp-tools */

(function (win, udef){
  ////// Import //////
  
  var arrp = $.arrp;
  var udefp = $.udefp;
  var inp = $.inp;
  var al = $.al;
  var len = $.len;
  var lpos = $.lpos;
  var slc = $.slc;
  var rpl = $.rpl;
  var apd = $.apd;
  var has = $.has;
  var beg = $.beg;
  var end = $.end;
  var push = $.push;
  var mtch = $.mtch;
  var err = $.err;
  
  var nilp = Lisp.nilp;
  
  var car = Lisp.car;
  var cdr = Lisp.cdr;
  var cons = Lisp.cons;
  
  var tag = Lisp.tag;
  
  ////// Processing functions //////
  
  function rembds(a){
    if (beg(a, "#"))return slc(a, 2, len(a)-1);
    return slc(a, 1, len(a)-1);
  }
  
  ////// Parser //////
  
  function prs(a){
    var lis = prslis(a);
    if (nilp(cdr(lis)))return car(lis);
    return cons("do", lis);
  }
  
  function prs1(a){
    if (isParen(a))return prslis(rembds(a));
    if (isSBrac(a))return prs1("(fn (_) (" + rembds(a) + "))");
    if (isBrac(a))return prs1("(arr " + rembds(a) + ")");
    if (isSParen(a))return prs1("(# " + rembds(a) + ")");
    if (isStr(a))return tag("str", JSON.parse(a));
    if (isRgx(a))return prsrgx(a);
    if (isCurl(a))return prs1("(obj " + rembds(a) + ")");
    if (beg(a, "'"))return prs1("(qt " + obj(a, 1) + ")");
    if (beg(a, "`"))return prs1("(qq " + obj(a, 1) + ")");
    if (beg(a, ",@"))return prs1("(uqs " + obj(a, 2) + ")");
    if (beg(a, ","))return prs1("(uq " + obj(a, 1) + ")");
    if (beg(a, "!"))return prs1("(not " + obj(a, 1) + ")");
    return a;
  }
  
  function prslis(a, pos){
    if (udefp(pos))pos = 0;
    var o = next(a, pos);
    if (o[1] == "")return [];
    if (o[1] == "." && pos != 0)return dot(a, o[0]+len(o[1]));
    return cons(prs1(o[1]), prslis(a, o[0]+len(o[1])));
  }
  
  function dot(a, pos){
    var o = next(a, pos);
    if (o[1] == "")err(dot, "Missing object after \".\" in a = $1", a);
    var n = next(a, o[0]+len(o[1]));
    if (n[1] != "")err(dot, "More than one object after \".\" in a = $1", a);
    return prs(o[1]);
  }
  
  function prsrgx(a){
    var s = rpl("\\\"", "\"", rembds(a));
    return new RegExp(s, "g");
  }
  
  function next(a, pos){
    var o = obj(a, pos);
    if (isSpace(o))return next(a, pos+len(o));
    return [pos, o];
  }
  
  function obj(a, pos){
    a = slc(a, pos);
    if (beg(a, "{", "(", "[")){
      var ds = {"(": ")", "[": "]", "{": "}"};
      var d = [a[0], ds[a[0]]];
      var p = 1; // paren lvl
      var s = false; // string lvl
      for (var i = 1; i < len(a); i++){
        switch (a[i]){
          case d[0]: if (!s)p++; break;
          case d[1]: if (!s)p--; break;
          case "\"": if (a[i-1] != "\\")s = !s; break;
        }
        if (p == 0)return slc(a, 0, i+1);
      }
      err(obj, "Brackets not matched in a = $1", a);
    }
    if (beg(a, "\"")){
      for (var i = 1; i < len(a); i++){
        if (a[i] == "\"" && a[i-1] != "\\"){
          return slc(a, 0, i+1);
        }
      }
      err(obj, "Double quotes not matched in a = $1", a);
    }
    if (beg(a, ",@"))return ",@" + obj(a, 2);
    if (beg(a, "#\"", "#[", "#(", "'", "`", "~", "!"))return a[0] + obj(a, 1);
    if (beg(a, ";"))return mtch(/^[^\n\r]*/, a);
    if (beg(a, /[\s]/))return mtch(/^[\s]+/, a);
    return mtch(/^[^\s]*/, a);
  }
  
  function isParen(a){
    return beg(a, "(") && end(a, ")");
  }
  
  function isSParen(a){
    return beg(a, "#(") && end(a, ")");
  }
  
  function isBrac(a){
    return beg(a, "[") && end(a, "]");
  }
  
  function isSBrac(a){
    return beg(a, "#[") && end(a, "]");
  }
  
  function isCurl(a){
    return beg(a, "{") && end(a, "}");
  }
  
  function isStr(a){
    return beg(a, "\"") && end(a, "\"");
  }
  
  function isRgx(a){
    return beg(a, "#\"") && end(a, "\"");
  }
  
  function isNum(a){
    return has(/^-?[0-9]+(\.[0-9]+)?$/, a);
  }
  
  function isSpace(a){
    return has(/^[\s,]+$/, a) || has(/^;[^\n\r]*$/, a);
  }
  
  ////// Object exposure //////
  
  apd({
    prs: prs
  }, Lisp);
  
  ////// Testing //////
  
  //al(prs("(+ 2 3)"));
  
})(window);
